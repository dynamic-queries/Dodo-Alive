function hip_trajectory = func_com_to_hip(x_phases, length_links, mass_links, mass_toe, mass_body, init_angles)
...
for p = 1:num_phases
    ...
    if phase_type
        traj{p} = com_traj(:,1) + flight_offset; %broadcasting?
    else
        for i = 1:length(com_traj)
            d = com_traj(i);
            traj{p}(i) = com_traj(i,1) + offset(d);
        end
    end
end
